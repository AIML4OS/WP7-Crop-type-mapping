...
#### Watershed / MRS (`python_mrs_summed`)
...
* **Algorithm options**:
  - **Option A (ANN-SAM / LPIS / MRS / Felzenszwalb / SLIC / OTB)** - `1_classify_ann_sam.py`: The primary neural network classifier script. It supports 6 segmentation methods selectable interactively in the CLI or starting directly using `--seg_mode`:
    ...
    - **`--seg_mode mrs_summed`** / **`otb_meanshift`** / **`felzenszwalb`** / **`slic`**: Launches the pipeline pointing directly to the corresponding dynamic segmentation files.
