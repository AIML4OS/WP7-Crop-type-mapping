...
# python 1_classify_presto_sar.py --track NL/orbit_88
# python 1_classify_presto_sar.py --track PT/orbit_161
...
class ProcessingPipeline:
    def __init__(self, track, seg_mode='sam'):
        ...
        print(f"Initializing Presto-SAR pipeline for Track: {self.track}, Country: {self.country}, Mode: {self.seg_mode}")
    ...
    def update_paths(self, method_name):
        ...
        self.sel_csv = self.samples_dir / f"{self.file_prefix}_presto_learn_features{suffix}.csv"
        self.class_tif = self.class_dir / f"{self.file_prefix}_presto_classified{suffix}.tif"
        self.conf_tif = self.class_dir / f"{self.file_prefix}_presto_confidence_map{suffix}.tif"
        self.masked_class = self.class_dir / f"{self.file_prefix}_presto_classified_masked{suffix}.tif"
        self.masked_conf = self.class_dir / f"{self.file_prefix}_presto_confidence_masked{suffix}.tif"
        self.metrics_fp = self.class_dir / f"{self.file_prefix}_presto_metrics{suffix}.xlsx"
    ...
    # --- Stage 3: Feature Extraction (Presto) ---
    def stage_3_selection(self):
        stage = 3
        if self.sel_csv.exists():
            print(f"[Stage {stage}] Presto features already extracted, skipping.")
            return

        print(f"[Stage {stage}/{self.total_stages}] Extracting deep Presto features for training segments...")
        ...
        print(f"    Presto Features saved to {self.sel_csv}\n")
    ...
    def stage_4_train_classifier(self, **kwargs):
        ...
        model_fn = self.model_dir / f"{self.file_prefix}_presto_model.pkl"
        print(f"[Stage {stage}/{self.total_stages}] Training Classifier (ANN) on Presto-SAR Embeddings...")
    ...
    # --- Stage 5: Tiled Inference (Presto + ANN) ---
    def stage_5_classify_vector(self, force_recompute=False):
        self._ensure_directories()
        stage = 5

        model_file = self.model_dir / f"{self.file_prefix}_presto_model.pkl"
        ...
        print(f"[Stage {stage}/{self.total_stages}] Running Object-Based Inference using Presto...")
        ...
        print("    Calculating Bayesian priors for Presto inference...")
...
def main_menu(pipeline):
    while True:
        ...
        menu = f"""
    --- Raster-Based OBIA Pipeline (Presto-SAR) ---
    ...
    [3] Stage 3: Extract Presto-SAR Features
...
def main():
    parser = argparse.ArgumentParser(description="Object-Based Crop Classification using NASA Harvest Presto")
