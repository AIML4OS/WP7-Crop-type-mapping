...
class ProcessingPipeline:
    ...
    def stage_7_calculate_metrics(self):
        ...
        sh.cell(row=2, column=1, value=f"Track: {self.track} ({self.country}) | Segmentation: {self.seg_mode.upper()} | Model: Presto-SAR Classifier").font = bold_font
        sh.cell(row=3, column=1, value="Data: Sentinel-1 SAR (Sigma0 VH/VV) + NASA Harvest Presto-SAR Embeddings").font = Font(name="Calibri", size=10, italic=True, color="595959")
        ...
        sh.cell(row=8, column=2, value=f"{len(true_labels):,}".replace(',', ' ')).font = regular_font
        ...
        for idx, c in enumerate(labels):
            ...
            c_samples = int(np.sum(true_labels == c))
