...
class ProcessingPipeline:
    ...
    def _apply_mask(self, input_tif, mask_tif, out_tif, stage):
        print(f"[Stage {stage}/{self.total_stages}] Applying Arable Mask...")
        
        if not mask_tif.exists():
            print(f"    WARNING: Arable mask not found at {mask_tif}. Copying input to output without masking.")
            shutil.copy(str(input_tif), str(out_tif))
            return
        ...
        for y in range(0, rows, tile_size):
            for x in range(0, cols, tile_size):
                ...
                # Apply mask: anything < 0.5 in the mask becomes NoData
    ...
    def stage_5_classify_vector(self, force_recompute=False):
        ...
        if self.class_tif.exists() and not force_recompute:
            ...
            print("   (To overwrite, select 'A' from main menu and ensure force=True, or delete the file manually).")
            return
        ...
        ds_cls.GetRasterBand(1).SetNoDataValue(0) # 0 is NoData
    ...
    def stage_8_calculate_metrics(self):
        ...
        if not self.metrics_fp.exists():
            ...
            # --- FIX: ALIGN CRS FOR CONTROL POINTS ---
            ...
            for _, row in ctrl.iterrows():
                try:
                    ...
                    if 0 <= px < ds.RasterXSize and 0 <= py < ds.RasterYSize:
                        t = int(row['crop_id'])
                        val_arr = band.ReadAsArray(px, py, 1, 1) # Read pixel safely
                        if val_arr is not None:
                            p = int(val_arr[0, 0])
                            # Treat 0 and -9999 as nodata/unclassified
            ...
            # Calculate areas (requires full array read, but masked class is compressed and integer, so usually fits)
    ...
    while True:
        ...
        try:
            if choice == '1':
                ...
                pipeline.stage_6_mask_class()
            elif choice == '7':
                pipeline.stage_7_mask_confidence()
            elif choice == '8':
                pipeline.stage_8_calculate_metrics()
            elif choice == 'A':
                print("\nNOTE: Running 'A' will automatically force recomputation of Stages 5, 6, and 7 to clear any corrupted old files.")
                pipeline.stage_1_segmentation(**pipeline.stage1_params)
                pipeline.stage_2_split_samples(**pipeline.stage2_params)
                pipeline.stage_3_selection()
                ...
                # If we forced recomputation, delete old metrics so it recalculates
